...
namespace Oxide.Ext.NoSteam.Patch
{
    ...
    /// <summary>
    ///     HookManager. Used to hook and unhook all hooked functions in a process. No method in this class is thread safe and
    ///     should be shared among all threads (protected by a Lock).
    ...
    internal class HookManager
    {
        ...
        /// <summary>
        ///     Creates a HookManager. HookManager is not thread safe. If it is to be used from multiple threads, one instance
        ///     should be shared among all threads
        ///     with a lock.
        /// </summary>
        ...
        /// <summary>
        ///     Replaces the method call original with the method replacement with a standard x86/x64 JMP hook. The methods do not
        ///     have to be in the same Assembly.
        ///     Original may be static, but replacement MUST be static and accept the same arguments (if the hooked method is
        ///     non-static, the first argument should be
        ///     of the class type).
        /// </summary>
        ...
        ///     If original and replacement are the same function, original is generic, replacement
        ///     is generic or non-static, or if the target function is already hooked.
        /// </exception>
        /// <exception cref="Win32Exception">If a native call fails. This is unrecoverable.</exception>
        public void Hook(MethodInfo original, MethodInfo replacement)
        {
            ...
            if (original == (object)replacement)
                throw
                    new ArgumentException(
                        "A function can't hook itself"); //original and replacement cannot be the same function

            //Hook sanity checks.
            if (replacement.IsGenericMethod)
                throw new ArgumentException(
                    "Hook method must be static and non-generic"); //hook method must not be static and non-generic
            if (hooks.ContainsKey(original))
                throw new ArgumentException(
                    "Attempting to hook an already hooked method"); //a method that is already hooked cannot be hooked again

            //hook function via JMP method and save the original opcodes to restore later
            var originalOpcodes = PatchJMP(original, replacement);
        ...
        /// <exception cref="ArgumentNullException">If original or replacement are null.</exception>
        /// <exception cref="Win32Exception">If a native call fails. This is unrecoverable.</exception>
        public void Unhook(MethodInfo original)
        {
            ...
            var originalOpcodes = hooks[original];
        ...
        private unsafe byte[] PatchJMP(MethodInfo original, MethodInfo replacement)
        {
            ...
            var oldProtecton = VirtualProtect(originalSite, (uint)originalOpcodes.Length,
                (uint)Natives.PageProtection.PAGE_EXECUTE_READWRITE);
            ...
            for (var k = 0; k < offset; k++) originalOpcodes[k] = *(originalSitePointer + k);
        ...
        private unsafe void UnhookJMP(MethodInfo original, byte[] originalOpcodes)
        {
            ...
            for (var k = 0; k < originalOpcodes.Length; k++)
